...
public class Chest_Later extends Later {
	...
	private static void generate(AsyncWorldEditor world, Chunk chunk, final Random random, BasicChest chest,
			boolean enable, Coord pos) {
		...
		if (enable)
			chest.placeChunk(chunk, x, y, z, random);
	...
	public static boolean generate_later(AsyncWorldEditor world, Random random, Coord coords, boolean enable,
			BasicChest chest) {
//        Bukkit.getLogger().info("1 " + coords.toString());
