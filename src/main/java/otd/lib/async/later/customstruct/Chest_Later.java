...
public class Chest_Later extends Later {
	...
	public static boolean generate_later(AsyncWorldEditor world, Coord coord, Random random, Material chest,
			...
			for (LootNode node : dungeon.loots) {
				if (random.nextDouble() < node.chance) {
					...
					if (node.min == node.max)
						...
						amount = node.min + random.nextInt(node.max - node.min + 1);
