...
public class ZoneWorld {
	...
	private int min_y = Integer.MAX_VALUE, max_y = Integer.MIN_VALUE;
	private int min_x = Integer.MAX_VALUE, max_x = Integer.MIN_VALUE;
	private int min_z = Integer.MAX_VALUE, max_z = Integer.MIN_VALUE;
	...
	static {
		...
		for (Material mat : Material.values()) {
			...
			if (tmp.contains("FENCE") && !tmp.contains("FENCE_GATE"))
				PHY_BLOCKS.add(mat);
			if (tmp.contains("GLASS_PANE"))
				PHY_BLOCKS.add(mat);
			if (tmp.contains("IRON_BARS"))
				PHY_BLOCKS.add(mat);
		}
	}
			...
			if (true) {
				...
				if (pos[0] >= minX && pos[0] <= maxX && pos[2] >= minZ && pos[2] <= maxZ) {
					...
					if (entry.getValue().data != null)
						cn.bd = entry.getValue().data;
					else
						cn.material = entry.getValue().material;
...
//	            }
	...
	public Material getTypeRaw(int x, int y, int z) {
		...
		if (bb.data != null)
			return bb.data.getMaterial();
		return bb.material;
	}

	public Material getTypeRawAir(int x, int y, int z) {
		String key = xyzToKey(x, y, z);
		if (!map.containsKey(key)) {
			return Material.AIR;
		}
	...
	public BlockData getData(int x, int y, int z) {
		String key = xyzToKey(x, y, z);
		if (!map.containsKey(key))
			return Bukkit.createBlockData(default_state);
