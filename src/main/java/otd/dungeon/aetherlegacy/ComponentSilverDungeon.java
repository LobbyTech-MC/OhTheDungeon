...
//        public static Material aercloud = Material.PACKED_ICE;
	...
	public static void generateClouds(AetherStructure structure, Material block, boolean isFlat, int cloudSize,
			int offsetX, int offsetY, int offsetZ, int xTendency, int zTendency) {
		int x = offsetX;
		int y = offsetY;
		int z = offsetZ;
				...
				for (int y1 = y; y1 < y + structure.random.nextInt(1) + 2; ++y1) {
					for (int z1 = z; z1 < z + structure.random.nextInt(4) + 3 * (isFlat ? 3 : 1); ++z1) {
						if (structure.getBlockState(x1, y1, z1) == Material.AIR) {
							if (Math.abs(x1 - x) + Math.abs(y1 - y) + Math.abs(z1 - z) < 4 * (isFlat ? 3 : 1)
									+ structure.random.nextInt(2)) {
	...
	public boolean generate(boolean cloud, Material cloud_material) {
		...
		if (cloud) {
			for (int tries = 0; tries < 100; tries++) {
				generateClouds(this, cloud_material, false, 10, this.random.nextInt(77), 0, this.random.nextInt(50),
						this.xTendency, this.zTendency);
			}
		}
						...
						if (type >= 3) {
							...
							case 1: {
								...
								if (this.getBlockState(u, 5 * q + 1, v) == Material.AIR) {
									ICoords chestCoords = new Coords(u + this.startX, 5 * q + 1 + this.startY,
											v + this.startZ);
									Chest_Later.generate_later(this.worldObj, random, chestCoords, Rarity.RARE,
											Material.CHEST);
								}
							...
							case 2: {
								...
								if (this.random.nextBoolean()) {
									// this.setBlockWithOffset(7 + 7 * p + this.random.nextInt(2), 5 * q + 1, 7 + 7
									// * r + this.random.nextInt(2), BlocksAether.chest_mimic);
