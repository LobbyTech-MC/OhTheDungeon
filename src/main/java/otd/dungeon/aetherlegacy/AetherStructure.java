...
public abstract class AetherStructure {
	...
	public boolean isBoxSolid(int x, int y, int z, int xRange, int yRange, int zRange) {
		...
		for (int lineX = x; lineX < x + xRange; lineX++) {
			for (int lineY = y; lineY < y + yRange; lineY++) {
				for (int lineZ = z; lineZ < z + zRange; lineZ++) {
					if (this.getBlockState(lineX + this.startX, lineY + this.startY,
							lineZ + this.startZ) == Material.AIR) {
						flag = false;
					}
				}
			}
	...
	public void setBlockWithOffset(int x, int y, int z) {
		...
		if (this.random.nextInt(this.chance) == 0) {
			this.placeBlockAtCurrentPosition(this.worldObj, this.extraBlockState, x + this.startX, y + this.startY,
					z + this.startZ);
		} else {
			this.placeBlockAtCurrentPosition(this.worldObj, this.blockState, x + this.startX, y + this.startY,
					z + this.startZ);
		}
	}
	...
	public boolean addComponentParts(AsyncWorldEditor worldIn, Random randomIn, boolean cloud,
