								...
								if ((mat4 == Material.GRASS_BLOCK)) {
									if (yy >= 1 && yy + var6 + 1 <= 256) {
										try (EditSession editSession = WorldEdit.getInstance().newEditSessionBuilder()
											...
											BlockVector3 position = BlockVector3.at(xx, yy - 1, zz);
											editSession.setBlock(position, BlockTypes.DIRT);
											position = BlockVector3.at(xx - 1, yy - 1, zz);
											editSession.setBlock(position, BlockTypes.DIRT);
											position = BlockVector3.at(xx, yy - 1, zz - 1);
											editSession.setBlock(position, BlockTypes.DIRT);
											position = BlockVector3.at(xx - 1, yy - 1, zz - 1);
											editSession.setBlock(position, BlockTypes.DIRT);
												...
												for (random0 = xx - rand; random0 <= xx + rand; ++random0) {
													...
													for (int random2 = zz - rand; random2 <= zz + rand; ++random2) {
														...
														if ((Math.abs(random1) != rand || Math.abs(random3) != rand || rand <= 0)
															...
															BlockState blockState = BukkitAdapter.adapt(leave.createBlockState().getBlockData());
															position = BlockVector3.at(random0, world9, random2);
															editSession.setBlock(position, blockState);
															position = BlockVector3.at(random0 - 1, world9, random2);
															editSession.setBlock(position, blockState);
															position = BlockVector3.at(random0, world9, random2 - 1);
															editSession.setBlock(position, blockState);
															position = BlockVector3.at(random0 - 1, world9, random2 - 1);
															editSession.setBlock(position, blockState);
											...
											for (world9 = 0; world9 < var6 - world8; ++world9) {
												...
												if (mrandom0 == Material.AIR || mrandom0 == Material.SPRUCE_LEAVES) {
													position = BlockVector3.at(xx, yy + world9, zz);
													editSession.setBlock(position, BlockTypes.SPRUCE_LOG);
												    position = BlockVector3.at(xx - 1, yy + world9, zz);
													editSession.setBlock(position, BlockTypes.SPRUCE_LOG);
													position = BlockVector3.at(xx, yy + world9, zz - 1);
													editSession.setBlock(position, BlockTypes.SPRUCE_LOG);
													position = BlockVector3.at(xx - 1, yy + world9, zz - 1);
													editSession.setBlock(position, BlockTypes.SPRUCE_LOG);
													/*
