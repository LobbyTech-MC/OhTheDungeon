...
public class WorldGenCherry1 {
	...
	void genTreeLayer(int par1, int par2, int par3, float par4, byte par5, Material par6) {
		...
		for (var11[par5] = var10[par5]; var12 <= var7; ++var12) {
			...
			while (var13 <= var7) {
				...
				if (var15 > par4 * par4) {
					...
					Material var14 = worldObj.getBlockId(var11[0], var11[1], var11[2]);

					if (var14 != Material.AIR && var14 != Material.PINK_WOOL) {
						++var13;
					} else {
						setBlockAndMetadata(worldObj, var11[0], var11[1], var11[2], par6, 1);
						++var13;
					}
				}
			}
	...
	private static void setBlockAndMetadata(IWorld world, int x, int y, int z, Material material, int extra) {
		world.setBlockAt(x, y, z, material);
	}
	...
	void generateLeafNode(int par1, int par2, int par3) {
		...
		for (int var5 = par2 + leafDistanceLimit; var4 < var5; ++var4) {
			...
			this.genTreeLayer(par1, var4, par3, var6, (byte) 1, Material.PINK_WOOL);
		}
	}
	 ...
	 */
	void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger, Material par3) {
	...
	void generateTrunk() {
		...
		this.placeBlockLine(var5, var6, Material.OAK_LOG);
	...
	void generateLeafNodeBases() {
		...
		for (int[] var3 = new int[] { basePos[0], basePos[1], basePos[2] }; var1 < var2; ++var1) {
			...
			if (this.leafNodeNeedsBase(var6)) {
				this.placeBlockLine(var3, var5, Material.OAK_LOG);
			}
		}
	}
	...
	int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger) {
		...
		if (var3[var5] == 0)
			...
			for (var15 = var3[var5] + var8; var14 != var15; var14 += var8) {
				...
				Material var16 = worldObj.getBlockId(var13[0], var13[1], var13[2]);

				if (var16 != Material.AIR && var16 != Material.PINK_WOOL) {
					break;
				}
			}
	...
	boolean validTreeLocation() {
		...
		Material var3 = worldObj.getBlockId(basePos[0], basePos[1] - 1, basePos[2]);

		if (var3 != Material.GRASS_BLOCK && var3 != Material.DIRT)
			return false;
		else {
