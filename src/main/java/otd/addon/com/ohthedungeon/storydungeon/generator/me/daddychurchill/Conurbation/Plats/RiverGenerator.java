...
public class RiverGenerator extends WaterGenerator {
	...
	public void generateChunk(final ByteChunk chunk, final Random random, final int chunkX, final int chunkZ) {
		...
		if (this.noise.isDelta(chunkX, chunkZ)) {
			for (int x = 0; x < 16; ++x) {
				for (int z = 0; z < 16; ++z) {
					this.generateChunkColumn(chunk, chunkX, chunkZ, x, z,
							this.getDeltaGroundSurfaceY(chunkX, chunkZ, x, z));
				}
			}
		} else {
			for (int x = 0; x < 16; ++x) {
	...
	public int generateChunkColumn(final ByteChunk chunk, final int chunkX, final int chunkZ, final int blockX,
			final int blockZ) {
		final int blockY = this.noise.isDelta(chunkX, chunkZ)
				? this.getDeltaGroundSurfaceY(chunkX, chunkZ, blockX, blockZ)
				: this.getGroundSurfaceY(chunkX, chunkZ, blockX, blockZ);
	...
	public int getDeltaGroundSurfaceY(final int chunkX, final int chunkZ, final int blockX, final int blockZ) {
		return (this.generatorLake.getGroundSurfaceY(chunkX, chunkZ, blockX, blockZ)
				+ this.getGroundSurfaceY(chunkX, chunkZ, blockX, blockZ)) / 2;
	}
}
