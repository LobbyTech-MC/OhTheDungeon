...
public class CandyWorld extends BaseGenerator {
	...
	private void generateHeightmap(int x, int z) {
		...
		float f = 684.412F;
		float f1 = 684.412F;
		this.mainNoiseRegion = this.mainPerlinNoise.generateNoiseArray(this.mainNoiseRegion, x, 0, z, 5, 33, 5, f / 80D,
				f1 / 160D, f / 80D);
		this.minLimitRegion = this.minLimitPerlinNoise.generateNoiseArray(this.minLimitRegion, x, 0, z, 5, 33, 5,
				(double) f, (double) f1, (double) f);
		this.maxLimitRegion = this.maxLimitPerlinNoise.generateNoiseArray(this.maxLimitRegion, x, 0, z, 5, 33, 5,
				(double) f, (double) f1, (double) f);
		int i = 0;
		int j = 0;
