	...
	public AsyncChunk asyncGenerateChunkData(long seed, Random rand, int ChunkX, int ChunkZ) {
		...
		for (int x = 0; x < 16; ++x) {
			for (int z = 0; z < 16; ++z) {
				...
				int ch = (int) (c.noise((double) rx, (double) rz, 0.75, 0.25)
						+ g.noise((double) rx, (double) rz, 1.25, 0.75)
						+ bg.noise((double) rx, (double) rz, 0.25, 0.5) * 18.0 + 20.0);
				int eg = (int) (e.noise((double) rx, (double) rz, 0.5, 0.5)
						+ g.noise((double) rx, (double) rz, 1.25, 0.75) * 38.0 + 64.0);
				int ds = (int) (d.noise((double) rx, (double) rz, 0.25, 0.75)
						+ c.noise((double) rx, (double) rz, 0.75, 0.25) + g.noise((double) rx, (double) rz, 1.25, 0.75)
						+ bg.noise((double) rx, (double) rz, 0.25, 0.5) * 32.0 + 25.0);
				for (int y = 2; y < max; ++y) {
					if (y < h || y < bh || y < eg) {
