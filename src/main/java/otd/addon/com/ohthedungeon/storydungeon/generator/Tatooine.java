	...
	public AsyncChunk asyncGenerateChunkData(long seed, Random random, int chunkx, int chunkz) {
		...
		for (int z = 0; z < 16; z++) {
			for (int x = 0; x < 16; x++) {
				...
				/*
				 * // SANDSTONE Layer double sandstone = this.noiseGenerator.noise(rx, rz, 8,
				 * 23)*4; for(int i=1;i< (int)(sandstone);i++) { HothUtils.setPos(chunk, x,y,z,
				 * Material.SANDSTONE); y++; }
				 * 
				 * // SAND Layer double sand = 1+this.noiseGenerator.noise(rx, rz, 8, 43)*4;
				 * for(int i=0;i< (int)(sand);i++) { HothUtils.setPos(chunk, x,y,z,
				 * Material.SAND); y++; }
				 */
				...
				/*
				 * // ice Layer while(y<34+surfaceOffset) { HothUtils.setPos(chunk, x,y,z,
				 * Material.ICE); y++; }
				 * 
				 * double icel = this.noiseGenerator.noise(rx, rz, 3, 68)*8; for(int i=3;i<
				 * (int)(icel);i++) { HothUtils.setPos(chunk, x,y,z, Material.ICE); y++; }
				 */
				...
				double ice = factor
						* (this.noiseGenerator.noise(rx, rz, 4, 63) * 2 + this.noiseGenerator.noise(rx, rz, 10, 12))
						* 2.5;
