...
public abstract class WaterGenerator extends PlatGenerator {
	...
	protected int generateChunkColumn(final ByteChunk chunk, final int chunkX, final int chunkZ, final int blockX,
			final int blockZ, final int waterbedY) {
		...
		if (toNorth && toWest && !toSouth && !toEast) {
			...
			if (toNorth) {
				...
				for (int x3 = 0; x3 < 16; ++x3) {
					...
					if (addStairs && x3 == 1) {
						chunk.setBlocks(x3, this.seabedLevel, y2 + 2 + seawallOffset - 1, 0,
								WaterGenerator.byteSeawall);
					} else {
			...
			if (toSouth) {
				...
				for (int x3 = 0; x3 < 16; ++x3) {
					...
					if (addStairs && x3 == 1) {
						chunk.setBlocks(x3, this.seabedLevel, y2 + 2 + seawallOffset - 1, 15,
								WaterGenerator.byteSeawall);
					} else {
			...
			if (toWest) {
				...
				for (int z3 = 0; z3 < 16; ++z3) {
					...
					if (addStairs && z3 == 1) {
						chunk.setBlocks(0, this.seabedLevel, y2 + 2 + seawallOffset - 1, z3,
								WaterGenerator.byteSeawall);
					} else {
			...
			if (toEast) {
				...
				for (int z3 = 0; z3 < 16; ++z3) {
					...
					if (addStairs && z3 == 1) {
						chunk.setBlocks(15, this.seabedLevel, y2 + 2 + seawallOffset - 1, z3,
								WaterGenerator.byteSeawall);
					} else {
