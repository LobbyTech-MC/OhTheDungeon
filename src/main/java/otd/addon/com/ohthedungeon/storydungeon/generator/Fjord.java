...
public class Fjord extends BaseGenerator {
	...
	// This converts relative chunk locations to bytes that can be written to the
	// chunk
	public int xyzToByte(int x, int y, int z) {
		return (x * 16 + z) * 128 + y;
	}

	// Set a block in a chunk. If the section in which the coordinates exist is not
	// allocated,
	// we allocate it here.
	...
	// Compared to generateBlockSections, this function has extended block ID
	// support. That means that the block ID
	// range is [0..4095] (note the fact that the functions returns a 2D array of
	...
	public AsyncChunk asyncGenerateChunkData(long seed, Random random, int x, int z) {
		...
		for (int bZ = 0; bZ < 16; ++bZ) {
			for (int bX = 0; bX < 16; ++bX) {
				...
				if (plainsValue <= 0.5) {
					...
					if (plainsValue >= 0.35) {
						// Between 0.5 and 0.35 we want to smoothly change the terrain instead of having
						// a sudden change at 0.5.
						lerpFactor = Util.LerpFactor(0.5f, 0.35f, (float) plainsValue);
					}
					noiseValue = sealevel * Util.Lerp(0.0f, 1.0f, lerpFactor)
							+ noiseValue * Util.Lerp(1.0f, 0.1f, lerpFactor);
				}
				...
				// Set a preliminary biome based on terrain height. May chance in the code that
				// follows.
				int taigaBorder = 95;
				// biomes.setBiome(bX, bZ, noiseValue > taigaBorder ? Biome.TAIGA_HILLS :
				// Biome.MOUNTAINS);
		...
		return result;
		// return null; // Default - returns null, which drives call to
...
//        result.setBlock(0, 0, 0, Material.GLASS);
	...
	private void DoLayer_Shore(AsyncChunk result, Random random, int bX, int bZ, int sealevel) {
		...
		// TODO MAYBE: Place gravel below it up until the next solid block so that it
		// doesn't fall down?
	}
	...
	private void DoLayer_SeaBed(AsyncChunk result, Random random, int bX, int bZ, int sealevel) {
		...
		if (highestBlockY <= sealevel) {
			for (int i = highestBlockY; i > highestBlockY - 5 && i > 1; --i) {
				...
				if (currBlockMat != Material.AIR && currBlockMat != Material.GRAVEL) {
	...
	private void DoLayer_Snow(AsyncChunk result, Random random, int bX, int bZ, int taigaBorder) {
		...
		int snowTransitionHeight = snowMinTransitionHeight
				+ (int) (random.nextGaussian() * (snowMaxTransitionHeight - snowMinTransitionHeight));
