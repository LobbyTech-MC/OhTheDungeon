...
/**
 * This handles the determination of which themes to use in each biome.
 * 
 * Included a HashSets for various Forge biome dictionary types, storing which
 * dungeons should appear in each. Additional sets are used to store with themes
 * should never appear in each.
 * 
 * In a given biome the possible dungeon themes equate to the union of inclusion
 * defining sets for types that describe it followed by the subtraction of the
 * union of the sets of forbidden themes for those types.
 * 
...
public class BiomeSets {
	 ...
	 * 
	 * This should probably been handled with a collection, most likely a HashMap
	 * between the sets and the enum constants used (or else their string
	 * representation.
	 */
	...
	/**
	 * This will look at a themes biome type data and add it to the proper sets for
	 * both inclusion in those type.
	 * 
	 * @param theme
	 */
	...
	/**
	 * This will use a themes biome data to added it to set for exclusion from
	 * appearing in biomes of a type.
	 * 
	 * @param theme
	 */
	...
	/**
	 * This will perform the set logic to determine which themes are available in
	 * the given biome then randomly select one from that set.
	 * 
	 * @param biome
	 * @param random
	 * @return
	...
	public static Theme getTheme(Biome biome, Random random) {
		...
		if (BiomeDictionary.hasType(biome, Type.WATER) || BiomeDictionary.hasType(biome, Type.BEACH)) {
			set.addAll(water);
		}
