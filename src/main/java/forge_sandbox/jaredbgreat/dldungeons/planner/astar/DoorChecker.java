 ...
 * doors. Among other things it will remove the infamous doors to nowhere, sort
 * doors by destination room, pick a good door for each room to run the A*
 * connectively text-fix on, eliminate doors the transition between liquid and
 * non-liquid tiles, and place liquids in doors that have liquid on both sides.
 * 
...
public class DoorChecker {
	...
	/**
	 * This will check to see if a tile is a valid location in the Dungeon. That is,
	 * it will check to see if its both inside the dungeons and has a room value
	 * other than zero.
	 * 
	 * @param tile
	...
	/**
	 * Will check if a tile representing a door location actually connects two
	 * rooms, and remove doors that do not. Put differently this will remove a door
	 * to nowhere and inform the calling method if such an action was needed.
	 * 
	 * @param door
	...
	public static boolean validateDoor(Dungeon dungeon, Doorway door) {
		return (validateTile(dungeon, door.x - 1, door.z) && validateTile(dungeon, door.x + 1, door.z)
				&& validateTile(dungeon, door.x, door.z - 1) && validateTile(dungeon, door.x, door.z + 1)
				&& validateTile(dungeon, door.x - 1, door.z - 1) && validateTile(dungeon, door.x + 1, door.z + 1)
				&& validateTile(dungeon, door.x + 1, door.z - 1) && validateTile(dungeon, door.x - 1, door.z + 1));
	}

	/**
	 * This will get a list of doors to run A* on for the purpose of ensuring room
	 * passibility.
	 * 
	 * @param room
	...
	/**
	 * This will run A* on exits to ensure all can be reached. It will also ensure
	 * the same door is used in connected rooms by giving a negative value and
	 * passing it to the connected rooms DoorQueue. *
	 * 
	 * @param exits
	 */
	...
	public static void processDoors2(Dungeon dungeon, Room room) {
		...
		for (Doorway exit : room.topDoors) {
			exit.priority -= 16;
			dungeon.rooms.get(getOtherRoom(exit, room, dungeon)).addToConnections(new Doorway(exit, room.id));
		}
	}
