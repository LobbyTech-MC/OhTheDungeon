...
public class AStar {
	...
	/**
	 * This is what actually runs A* (and also calls other methods to make practical
	 * use of the results).
	 */
	public void seek() {
		Step current;
	...
	/**
	 * This uses the data from AStar to make useful changes to the dungeon.
	 * 
	 * @param end
	 */
	public void makeRoute(Step end) {
		...
		if (dungeon.map.isWall[end.x][end.z] || dungeon.map.isFence[end.x][end.z])
			dungeon.map.isDoor[end.x][end.z] = true;
		if (dungeon.map.hasLiquid[end.x][end.z]) {
			dungeon.map.hasLiquid[end.x][end.z] = false;
			dungeon.map.floorY[end.x][end.z] = (byte) dungeon.rooms.get(room).floorY;
		}

		do {
			dungeon.map.astared[child.x][child.z] = true;
			if (dungeon.map.isWall[child.x][child.z] || dungeon.map.isFence[child.x][child.z])
				addDoor(parent, child);
			if (dungeon.map.hasLiquid[child.x][child.z])
				fixLiquid(parent, child, (byte) dungeon.rooms.get(room).floorY);
		...
		if (dungeon.map.hasLiquid[child.x][child.z]) {
			...
			dungeon.map.floorY[child.x][child.z] = (byte) dungeon.rooms.get(room).floorY;
		}
	}
	...
	protected void fixLiquid(Step from, Step to, byte floorY) {
		...
		dungeon.map.floorY[to.x][to.z] = dungeon.map.floorY[from.x][from.z];
	...
	protected void fixHeights(Step from, Step to) {
		...
		if (diff > 2)
			dungeon.map.floorY[to.x][to.z] = (byte) (dungeon.map.floorY[from.x][from.z] - 1);
		if (diff < -2)
			dungeon.map.floorY[to.x][to.z] = (byte) (dungeon.map.floorY[from.x][from.z] + 1);
	}
