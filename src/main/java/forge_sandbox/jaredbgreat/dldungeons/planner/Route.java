 ...
 * find an actual concrete route and add it to the dungeon. To do that it will
 * produce a sequence of rooms from each end (start and finish), alternately
 * adding a room to each in the general direction of the other until the
 * squences connect at some point.
 * 
...
public class Route {
	...
	/**
	 * This will attempt to draw the connecting serious of rooms by calling
	 * drawConnection until either the connection is flagged as complete or the
	 * dungeon has reach the maximum number of rooms.
	 * 
	 * Note that the connection may be flagged complete either because an actual
	 * connection has been found or because both sequences (that from the start and
	 * that from the
	 * 
	 * @param dungeon
	 */
	...
	/**
	 * This will try to add a room to one of the sequences. Which sequence gets this
	 * turn is determined by the internal finishTurn variable, which alternates with
	 * each call. The location of the new rooms will always be closer on either the
	 * x or z axis, though which is determined randomly, though weighted to favor
	...
	public void drawConnection(Dungeon dungeon) {
		...
		if (close(dungeon.size.maxRoomSize - 1))
		...
		if (finishTurn) {
			...
			if ((temp == null) || side2.contains(temp))
				temp = current2.connector(dungeon, dir2, xdim, zdim, roomHeight, this);
			if ((temp == null) || side2.contains(temp)) {
			...
			if (temp == null || side1.contains(temp))
				temp = current1.connector(dungeon, dir2, xdim, zdim, roomHeight, this);
			if (temp == null || side1.contains(temp)) {
	 ...
	 * sequence. Specifically this finds which direction to move from the terminal
	 * room in the sequence to that in the finish sequence.
	 * 
	...
	private int touchDir() {
		if (zOverlap()) {
			if (current1.endX == current2.beginX)
				return 0;
			if (current1.beginX == current2.endX)
				return 2;
		} else if (xOverlap()) {
			if (current1.endZ == current2.beginZ)
				return 1;
			if (current1.beginZ == current2.endZ)
				return 3;
		}
	...
	private boolean close(int range) {
		if (Math.abs(current1.beginX - current2.endX) > range)
			return false;
		if (Math.abs(current2.beginX - current1.endX) > range)
			return false;
		if (Math.abs(current1.beginZ - current2.endZ) > range)
			return false;
		if (Math.abs(current2.beginZ - current1.endZ) > range)
			return false;
		return true;
	}
	 ...
	 * 
	 * The direction is picked in such a way that sequences attempt to "grow" toward
	 * each other while also maintaining a degree of randomness.
	 * 
	 * @param random
	 */
	private void getGrowthDir(Random random) {
		...
		if (xOverlap()) {
			...
			if (posX) {
				...
				.nextInt((int) (Math.abs(realXDist) + Math.abs(realZDist) + 1)) > (int) (Math.abs(realXDist))) {
			if (posX) {
				dir1 = 0;
				if (posZ)
					dir2 = 1;
