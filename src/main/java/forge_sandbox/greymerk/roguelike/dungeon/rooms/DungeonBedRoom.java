...
import forge_sandbox.greymerk.roguelike.worldgen.blocks.Bed;
...
public class DungeonBedRoom extends DungeonBase {
	...
	public boolean generate(IWorldEditor editor, Random rand, LevelSettings settings, Cardinal[] entrances,
		...
		if (rand.nextBoolean())
			cursor.add(Cardinal.reverse(dir));
		Furnace.generate(editor, new ItemStack(Material.COAL, 2 + rand.nextInt(3)), true, Cardinal.reverse(side),
				cursor);
