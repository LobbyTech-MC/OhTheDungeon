...
public enum Treasure {
	...
	ARMOUR, WEAPONS, BLOCKS, ENCHANTING, FOOD, ORE, POTIONS, STARTER, TOOLS, SUPPLIES, SMITH, MUSIC, REWARD, EMPTY,
	BREWING;
	...
	public static ITreasureChest generate(IWorldEditor editor, Random rand, Coord pos, Treasure type, int level,
			boolean trapped) throws ChestPlacementException {
	...
	public static boolean isValidChestSpace(IWorldEditor editor, Coord pos) {
		...
		for (Cardinal dir : Cardinal.directions) {
			cursor = new Coord(pos);
			cursor.add(dir);
			if (editor.getMaterial(cursor) == Material.CHEST)
				return false;
		}
