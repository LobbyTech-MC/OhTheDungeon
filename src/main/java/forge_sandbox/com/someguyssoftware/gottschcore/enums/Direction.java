...
public enum Direction implements IEnum {
	UP(0, "Up", Alignment.VERTICAL), DOWN(1, "Down", Alignment.VERTICAL), NORTH(2, "North", Alignment.HORIZONTAL),
	EAST(3, "East", Alignment.HORIZONTAL), SOUTH(4, "South", Alignment.HORIZONTAL),
	...
	public Rotate getRotation(Direction direction) {
		...
		case NORTH: // <-- destination
			switch (this) {
			case NORTH:
				return Rotate.NO_ROTATE; // <-- source
			case EAST:
	 ...
	 * @return
	 */
	public static Direction fromFacing(BlockFace facing) {
		switch (facing) {
		case NORTH:
			return Direction.NORTH;
		case EAST:
