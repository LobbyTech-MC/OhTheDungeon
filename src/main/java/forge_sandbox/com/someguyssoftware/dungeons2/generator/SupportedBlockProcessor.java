...
//    }
	...
	public int applySupportRulesPass1(final AsyncWorldEditor world, final ICoords indexCoords,
		...
		if (elem == DesignElement.AIR || elem == DesignElement.SURFACE_AIR || elem == DesignElement.NONE
...
//                Plan.log.info("Total Amount of support coming with the HORIZONTAL(ZNeg) support: " + amount);
	 ...
	 * @return
	 */
	public int applySupportRulesPass2(final AsyncWorldEditor world, final ICoords indexCoords,
			final ICoords worldCoords, final DesignElement elem) {
		...
		// NOTE only HORIZONTAL support is calculated in Pass2 as VERTICAL was already
		// calculated in Pass1
	 ...
	 * @return
	 */
	public int getYNegSupportAmount(final AsyncWorldEditor world, final ICoords indexCoords, final ICoords worldCoords,
			DesignElement elem) {
...
//            Plan.log.info("YNeg World block " + worldBlockState.getBlock().getUnlocalizedName() + " is a supporting type block.");
			if (indexCoords.getY() == 0) {
				// since the floor (y ==0) is being processed AND there is a supporting world
				// block underneath, return 100 support for this location
				return 100;
			} else {
				/*
				 * Check if the worldBlockState was there originally or placed there by plans.
				 * This can be achieved by looking at the supportedBlocks matrix - if an entry
				 * is null, that indicates that that block was replaced/set by the processor -
				 * either it was a null_block or other preservation rule.
				 */
				ISupportedBlock supportBlock = this.supportMatrix[indexCoords.getY() - 1][indexCoords
						.getZ()][indexCoords.getX()];
...
//                    Plan.log.info("SupportedBlocks Matrix did not have entry - return 100.");
				...
				DesignElement supportElement = blockProvider.getDesignElement(worldCoords.add(0, -1, 0), room,
						room.getLayout());

				// get the support amount from the design element that is "behind" the index
				// coords
	 ...
	 * @return
	 */
	public int getXNegSupportAmount(final AsyncWorldEditor world, final ICoords indexCoords, final ICoords worldCoords,
			DesignElement elem) {
		int amount = 0;
...
//            Dungeons2.log.info("XNeg World block " + worldBlockState.getBlock().getUnlocalizedName() + " is a supporting type block.");
			if (indexCoords.getX() == 0) {
				...
				ISupportedBlock supportBlock = this.supportMatrix[indexCoords.getY()][indexCoords.getZ()][indexCoords
						.getX() - 1];
				if (supportBlock == null) {
					return 100;
				}
				...
				// get the design element @ the support position
				DesignElement supportElement = blockProvider.getDesignElement(worldCoords.add(-1, 0, 0), room,
						room.getLayout());
...
//                    Dungeons2.log.debug("Getting regular support amount for block:" + supportElement.getHorizontalSupport());
	 ...
	 * @return
	 */
	public int getZNegSupportAmount(final AsyncWorldEditor world, final ICoords indexCoords, final ICoords worldCoords,
			DesignElement elem) {
		int amount = 0;
		...
		if (worldBlockState != Material.AIR && isSupportingBlock(world, supportPos)) {
			if (indexCoords.getZ() == 0) {
				...
				ISupportedBlock supportBlock = this.supportMatrix[indexCoords.getY()][indexCoords.getZ()
						- 1][indexCoords.getX()];
				if (supportBlock == null) {
					return 100;
				}
				...
				// get the design element @ the support position
				DesignElement supportElement = blockProvider.getDesignElement(worldCoords.add(0, 0, -1), room,
						room.getLayout());
	 ...
	 * @return
	 */
	public int getXPosSupportAmount(final AsyncWorldEditor world, final ICoords indexCoords, final ICoords worldCoords,
			DesignElement elem) {
		int amount = 0;
		BlockPos supportPos = new BlockPos(worldCoords.getX() + 1, worldCoords.getY(), worldCoords.getZ());
		Material worldBlockState = world.getBlockState(supportPos);
...
//            Plan.log.info("XPos World block " + worldBlockState.getBlock().getUnlocalizedName() + " is a supporting type block.");
			if (indexCoords.getX() == getSupportMatrix()[0][0].length - 1) {
				...
				ISupportedBlock supportBlock = getSupportMatrix()[indexCoords.getY()][indexCoords.getZ()][indexCoords
						.getX() + 1];
				if (supportBlock == null) {
					return 100;
				}
	 ...
	 * @return
	 */
	public int getZPosSupportAmount(final AsyncWorldEditor world, final ICoords indexCoords, final ICoords worldCoords,
			DesignElement elem) {
		int amount = 0;
		BlockPos supportPos = new BlockPos(worldCoords.getX(), worldCoords.getY(), worldCoords.getZ() + 1);
		Material worldBlockState = world.getBlockState(supportPos);
		...
		if (worldBlockState != Material.AIR && isSupportingBlock(world, supportPos)) {
			if (indexCoords.getZ() == getSupportMatrix()[0].length - 1) {
				...
				ISupportedBlock supportBlock = getSupportMatrix()[indexCoords.getY()][indexCoords.getZ()
						+ 1][indexCoords.getX()];
				if (supportBlock == null) {
					return 100;
				}
	...
	/**
	 * Determines if the block is that type that can support other blocks. ie. a
	 * solid, non-replacable block
	 * 
	 * @param world
	 * @param blockPos
	 * @return
