...
public abstract class AbstractRoomGenerator implements IRoomGenerator {
	 ...
	 * and the room wall) and clear blocks for doorway. Must continue into room
	 * until there is an air block either to the front or to the sides. This check
	 * will remove any crown/trim/pillar blocks that might be in the way.
	 * 
	 * @param world
	 * @param coords
	 * @param direction
	...
	protected void buildDoorway(AsyncWorldEditor world, ICoords coords, Direction direction) {
		...
		do {
			...
			if (world.getBlockState(pos.north()) == Material.AIR)
				touching++;
			if (world.getBlockState(pos.south()) == Material.AIR)
				touching++;
			if (world.getBlockState(pos.east()) == Material.AIR)
				touching++;
			if (world.getBlockState(pos.west()) == Material.AIR)
				touching++;
