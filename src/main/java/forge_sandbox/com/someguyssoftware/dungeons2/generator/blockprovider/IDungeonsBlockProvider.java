...
public interface IDungeonsBlockProvider {
	...
	public final static BlockData NULL_BLOCK = Bukkit.createBlockData(Material.AIR);

	default public BlockData getBlockState(Random random, ICoords worldCoords, Room room, Arrangement arrangement,
	 ...
	 * @return
	 */
	default public BlockData getBlockState(Arrangement arrangement, Style style, int decayIndex) {
		String block;
		BlockData blockState = NULL_BLOCK;
		...
		blockState = Bukkit.createBlockData(block);

		// rotate block to the direction of Arrangement if rotatable type
		DesignElement elem = arrangement.getElement();
		// TODO FUTURE add property to DesignElement enum, ROTATABLE
		if (elem == DesignElement.CROWN || elem == DesignElement.TRIM || elem == DesignElement.CORNICE
				|| elem == DesignElement.PLINTH || elem == DesignElement.COLUMN || elem == DesignElement.CAPITAL
				|| elem == DesignElement.BASE || elem == DesignElement.PILASTER || elem == DesignElement.LADDER
				|| elem == DesignElement.GUTTER) {
			/*
			 * NOTE gutter didn't get rotated 180 because it is not CardinalDirectionFacade
			 * but just CardinalDirection. Nor should it be rotated because it should face
			 * into the wall).
			 */

			/*
			 * Since RelativeDirection and CardinalDirection are built is in opposite
			 * direction of Minecraft vanilla blocks, they have to be rotated 180 degrees
			 * from what the direction originally is.
			 */
			Direction d;
	...
	default public Arrangement getArrangement(ICoords coords, Room room, Layout layout) {
		// determine the design element of the block @ xyz represents: floor, wall,
		// ceiling, crown, trim, cornice, base, pillar door, air, etc
	...
	default public Direction getDirection(ICoords coords, Room room, DesignElement element, Location location) {
		...
		// TEMP exterior elements need to face in the opposite direction as default
		// interior elements
		else if (
//                element.getFace() == Face.EXTERIOR || //<-- probably only need this one
		element == DesignElement.PLINTH || element == DesignElement.COLUMN || element == DesignElement.CORNICE
				|| element == DesignElement.COLUMN || element == DesignElement.CAPITAL
				|| element == DesignElement.BASE) {
			if (coords.getX() == room.getMinX())
				direction = Direction.EAST;
			else if (coords.getX() == room.getMaxX())
				direction = Direction.WEST;
			else if (coords.getZ() == room.getMinZ())
				direction = Direction.SOUTH;
			else
				direction = Direction.NORTH;
		} else {
			/*
			 * using stairs as an example, the back (the tallest side) is the north side and
			 * faces north therefore if stairs are to be used as trim or cornice, then the
			 * direction needs to point in the same direction of the side. ie NORTH_SIDE =
			 * NORTH
			 */
			if (location == Location.SOUTH_SIDE)
				direction = Direction.SOUTH;
	...
	default public DesignElement getDesignElement(ICoords coords, Room room, Layout layout) {
		...
		if (isWallElement(coords, room, layout)) {
			if (isFacadeSupport(coords, room, layout))
				return DesignElement.FACADE_SUPPORT;
			if (room.hasWallBase() && isWallBase(coords, room, layout))
				return DesignElement.WALL_BASE;
			if (room.hasWallCapital() && isWallCapital(coords, room, layout))
		 ...
		 * Optional elements
		 */
		// check for pilaster. if a room has a pilaster and pillar, then pilaster is
		// used
		if (room.hasPilaster() && isPilasterElement(coords, room, layout)) {
			// determine if base, shaft or capital
			if (isBaseElement(coords, room, layout))
				return DesignElement.PILASTER_BASE;
			else if (isCapitalElement(coords, room, layout))
		...
		if (room.hasCrown() && isCrownElement(coords, room, layout))
			return DesignElement.CROWN;
		// check for trim
		if (room.hasTrim()) {
//            Dungeons2.log.debug("Room has Trim.");
			// &&
			if (isTrimElement(coords, room, layout))
...
//            Dungeons2.log.debug("Not a Trim element.");
		...
		if (room.hasCoffer() && isCofferedCrossbeamElement(coords, room, layout))
			return DesignElement.COFFERED_CROSSBEAM;
		if (room.hasCoffer() && isCofferedMidbeamElement(coords, room, layout))
			return DesignElement.COFFERED_MIDBEAM;

		// check for ceiling
		if (isCeilingElement(coords, room, layout))
		...
		if (isSurfaceAirElement(coords, room, layout)) {
			...
			// check for wall air before ceiling, so the corner edges are recorded as wall
			// as opposed to ceiling
			if (((x == room.getMinX() + 1 || x == room.getMaxX() - 1) && z > room.getMinZ() && z < room.getMaxZ())
					|| ((z == room.getMinZ() + 1 || z == room.getMaxZ() - 1) && x > room.getMinX()
							&& x < room.getMaxX()))
	...
	/**
	 * Return same value as crossbeam. TODO future - determine if midbeam properly.
	 * 
	 * @param coords
	 * @param room
	 * @param layout
	...
	default public boolean isSurfaceAirElement(ICoords coords, Room room, Layout layout) {
		return !(coords.getY() > room.getMinY() + 1 && coords.getY() < room.getMaxY() - 1
	...
	default public Style getStyle(DesignElement elem, Layout layout, Theme theme, StyleSheet styleSheet) {
		...
		/*
		 * any of the following elements should return without a style NOTE
		 * FACADE_SUPPORT should not be included in this list as it is a listed design
		 * element and returns a WALL!
		 */
		if (elem == null || elem == DesignElement.NONE || elem == DesignElement.AIR
				|| elem.getFamily() == DesignElement.SURFACE_AIR
		...
		// recursively check if layout has a ref and check if it contains the frame (up
		// to 5 nested refs)
		if (frame == null) {
			...
			while (frame == null && layout != null && layout.getRef() != null && layout.getRef().length() > 0) {
				if (layoutRefs.contains(layout.getRef())) {
					Dungeons2.log.warn(String.format("Stylesheet layout circular dependency: %s. Using defaults.",
							layout.getRef()));
					break;
...
//            Dungeons2.log.debug("Getting element from default stylesheet:" + elem.getName());
			frame = DungeonGenerator.getDefaultStyleSheet().getLayouts().get(Layout.DEFAULT_NAME).getFrames()
					.get(elem.name());
		}
...
//            Dungeons2.log.debug(String.format("theme: %s\nAlias:%s", theme.getName(), frame.getAlias()));
			// TODO need to catch error here is style == null or theme.getAliases == null or
			// the alias from the theme doesn't exist
			style = styleSheet.getStyles().get(theme.getAliases().get(frame.getAlias()).getStyle());
			if (style == null) {
				Dungeons2.log
						.warn(String.format("Unable to locate style based on theme [%s] and frame [%s]'s alias [%s]",
								theme.getName(), elem.name(), frame.getAlias()));
			}
...
//            Dungeons2.log.debug("style=" + style);
	...
	default public boolean isWallElement(ICoords coords, Room room, Layout layout) {
		return coords.getX() == room.getMinX() || coords.getZ() == room.getMinZ() || coords.getX() == room.getMaxX()
				|| coords.getZ() == room.getMaxZ();
	}
	...
	default public boolean isCrownElement(ICoords coords, Room room, Layout layout) {
		return coords.getY() == room.getMaxY() - 1 && room.getHeight() > 4
				&& (coords.getX() == room.getMinX() + 1 || coords.getX() == room.getMaxX() - 1
						|| coords.getZ() == room.getMinZ() + 1 || coords.getZ() == room.getMaxZ() - 1);
	}

	/**
	 * Checks for support block for crown and cornice. It is assumed that the
	 * element at x,y,z has already been determined to be a wall. This method simply
	 * checks the vertical position to determine if this element is the 1 less than
	 * the ceiling. No additional checks are made to ensure that is position is
	 * indeed a wall.
	 * 
	 * @param coords
	 * @param room
	...
	default public boolean isTrimElement(ICoords coords, Room room, Layout layout) {
		return /* layout.getFrames().get(DesignElement.TRIM.name()) != null && */
		/*
		 * room has to be at least 7x7 and 4 high to give enough space to walk around
		 */
		room.getWidth() >= 7 && room.getDepth() >= 7 && room.getHeight() > 4 && coords.getY() == room.getMinY() + 1
				&& (coords.getX() == room.getMinX() + 1 || coords.getX() == room.getMaxX() - 1
						|| coords.getZ() == room.getMinZ() + 1 || coords.getZ() == room.getMaxZ() - 1);
	}

	/**
	 * Default isLadder method is a ladder that in going down from the floor as it
	 * is assumed dungeons go down.
	 * 
	 * @param coords
	 * @param room
	 * @param layout
	...
	default public boolean isLadderElement(ICoords coords, Room room, Layout layout) {
		...
		// want the ladder on the opposite side the room is facing ie if room faces
		// north, want the ladder on the south side of pillar (so that the ladder still
		// faces north)
		switch (direction) {
		case NORTH:
			if (x == center.getX() && z == (center.getZ() + 1))
				return true;
			break;
		case EAST:
			if (x == (center.getX() - 1) && z == center.getZ())
				return true;
			break;
		case SOUTH:
			if (x == center.getX() && z == (center.getZ() - 1))
	...
	default public boolean isPillarElement(ICoords coords, Room room, Layout layout) {
		...
		// if the room also has pilasters, then the offset is increased so there is
		// still space between pillar and pilaster
		if (room.hasPilaster()) {
			offset += 2;
			remainder = 1;
		}

		return ((xIndex > offset && xIndex < room.getWidth() - offset) && Math.abs(xIndex % 3) == remainder
				&& zIndex > offset && zIndex < room.getDepth() - offset && Math.abs(zIndex % 3) == remainder);
	}
	...
	default public boolean isPilasterElement(ICoords coords, Room room, Layout layout) {
		...
		if ((room.getWidth() <= 5 || room.getDepth() <= 5) || coords.getY() == room.getMinY()
				|| coords.getY() == room.getMaxY())
			return false;
		...
		if (room.getWidth() % 2 == 0 || room.getDepth() % 2 == 0) {
			return (x == room.getMinX() + 1 || x == room.getMaxX() - 1)
					&& (z == room.getMinZ() + 1 || z == room.getMaxZ() - 1);
		}
		// odd x-z axis index

		return ((x == room.getMinX() + 1 || x == room.getMaxX() - 1) && Math.abs(zIndex % 2) == 1 && zIndex > 0
				&& zIndex < room.getDepth() - 1)
				|| ((z == room.getMinZ() + 1 || z == room.getMaxZ() - 1) && Math.abs(xIndex % 2) == 1 && xIndex > 0
						&& xIndex < room.getWidth() - 1);
	}
	...
	default public boolean isGutterElement(ICoords coords, Room room, Layout layout) {
		...
		// if on the floor level (min y)
		return coords.getY() == room.getMinY() &&
		// if on the inner edge of the wall and not the corners (gutter block isn't
		// setup yet for corners)
				(((x == room.getMinX() + 1 || x == room.getMaxX() - 1) && z > room.getMinZ() + 1
						&& z < room.getMaxZ() - 1)
						|| ((z == room.getMinZ() + 1 || z == room.getMaxZ() - 1) && x > room.getMinX() + 1
								&& x < room.getMaxX() - 1));
	}
	...
	/**
	 * It is assumed that the element at x,y,z has already been determined to be a
	 * column. This method simply checks the vertical position to determine if this
	 * element is the base of the column. No additional checks are made to ensure
	 ...
	 * It is assumed that the element at x,y,z has already been determined to be a
	 * column. This method simply checks the vertical position to determine if this
	 * element is the capital of the column. No additional checks are made to ensure
	 * that is position is indeed a column.
	 * 
	 * @param x
	 * @param y
