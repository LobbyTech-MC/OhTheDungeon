...
public class DungeonGenerator {
	// TODO either these shouldn't be static or they need to be loaded by static {},
	// not constructor or this class is singleton
	/*
	 ...
	 */
	public boolean generate(AsyncWorldEditor world, Random random, Dungeon dungeon, StyleSheet styleSheet,
			SpawnSheet spawnSheet) {
		...
		/*
		 * a layout assigner. it determine the layout to use for each room and what
		 * design elements to enable
		 */
		 ...
		 */
		// TODO is many more decorators, they all should share the same set of loaders
		// instead of creating new ones each.
		...
		/*
		 * NOTE careful here. IRoomGenerator can alter the state of the
		 * IGenerationStrategy with a IDungeonsBlockProvider of it's choosing. Don't
		 * share between generators or have to synchronize
		 */
		...
		for (Level level : dungeon.getLevels()) {
			Dungeons2.log.debug("Level -> {} ", new Object[] { levelCount });
			Theme levelTheme;
			...
			for (Room room : level.getRooms()) {
				...
				if (room.getType() == Type.BOSS) {
					...
					// TODO if going to integrate with Treasure2, need to know how big the dungeons
					// is: size, # of levels
					// in order to select the correct chest/loot table rarity
					// TODO also should make it randomized ex 50% normal boss chest, 50% treasure2
					// chest
					// extend the bossRoomDecorator -> TreasureBossRoomDecorator and set more
					// properties
					// TODO should return ICoords of boss chest so Dungeon can record
					bossRoomDecorator.decorate(world, random, dungeon,
							roomGen.getGenerationStrategy().getBlockProvider(), room, level.getConfig());
				}

				/*
				 * TODO this should be a random selection of special rooms TODO there should be
				 * something to describe where a special room can occur (ex levels 4-6) select
				 * any special decorators
				 */
				// ensure room fits the criteria to host a library
				else if (room.getWidth() > 5 && room.getDepth() > 5 && room.getHeight() >= 5 && !room.hasPillar()
						&& random.nextInt(100) < 10 && libraryCount < 3) {
					Dungeons2.log.debug("Using library decorator for room @ " + room.getCoords().toShortString());
					libraryDecorator.decorate(world, random, dungeon,
							roomGen.getGenerationStrategy().getBlockProvider(), room, level.getConfig());
					libraryCount++;
				} else {
					// decorate the room (spawners, chests, webbings, etc)
					roomDecorator.decorate(world, random, dungeon, roomGen.getGenerationStrategy().getBlockProvider(),
							room, level.getConfig());
				}
			...
			for (Hallway hallway : level.getHallways()) {
				...
				// NOTE passing hallways here is a list of hallways (excluding the current one,
				// to check if they intersect
				roomGen = factory.createHallwayGenerator(hallway, level.getRooms(), generatedHallways,
						level.getConfig().isSupport());
...
//                Dungeons2.log.debug("Building Shaft: " + shaft);
	 ...
	 * @param roomDecorator
	 */
	private void buildEntrance(AsyncWorldEditor world, Random random, Dungeon dungeon, LayoutAssigner layoutAssigner,
			RoomGeneratorFactory factory, IRoomDecorator roomDecorator, StyleSheet styleSheet) {
		...
		Dungeons2.log.debug("Levels.size -> {}", new Object[] { dungeon.getLevels().size() });
		...
		entranceLevelConfig.setDecayMultiplier(Math.min(5, entranceLevelConfig.getDecayMultiplier())); // increase the
																										// decay
																										// multiplier to
																										// a minimum of
																										// 5
		...
		IRoomGenerator roomGen = factory.createRoomGenerator(random, entranceRoom,
				dungeon.getLevels().get(0).getConfig().isSupport());
		// TODO need to provide the entrance room generator with a different level
		// config that uses a higher decay multiplier
		...
		roomDecorator.decorate(world, random, dungeon, roomGen.getGenerationStrategy().getBlockProvider(), entranceRoom,
				entranceLevelConfig);
	}
