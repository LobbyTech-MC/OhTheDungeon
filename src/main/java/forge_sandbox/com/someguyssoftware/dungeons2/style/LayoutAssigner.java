...
public class LayoutAssigner {
	 ...
	 * @param styleSheet
	 */
	public final void loadStyleSheet(StyleSheet styleSheet) {
		// clear the map
		map.clear();
...
//            Dungeons2.log.debug("Assigning Hallway layout:" + layout.getName());
	...
	private void assignLayoutToRoom(Random random, Layout layout, Room room) {
		// if useAll is set to true, then if the frame exists, activate the element in
		// the room
		if (layout.isUseAll()) {
			...
			if (getStyleSheet().hasFrame(layout, DesignElement.PILASTER) && room.getWidth() >= 5
					&& room.getDepth() >= 5) {
			...
			if (room.getType() == Type.ENTRANCE) {
				...
				if (getStyleSheet().hasFrame(layout, DesignElement.CRENELLATION) && room.getWidth() >= 7
						&& room.getDepth() >= 7) {
...
//                Dungeons2.log.debug("Room has Trim @ " + room.getCoords());
			...
			// coffer
			if ((getStyleSheet().hasFrame(layout, DesignElement.COFFERED_CROSSBEAM)
					|| getStyleSheet().hasFrame(layout, DesignElement.COFFERED_MIDBEAM)) && random.nextInt(100) < 20) {
				room.setHasCoffer(true);
			}
			...
			// entrance rooms
			if (room.getType() == Type.ENTRANCE) {
				if (room.getWidth() >= 7 && getStyleSheet().hasFrame(layout, DesignElement.PILASTER)
						&& random.nextInt(100) < 25) {
					room.setHasPilaster(true);
				} else {
				...
				if (getStyleSheet().hasFrame(layout, DesignElement.CRENELLATION) && room.getWidth() >= 7
						&& room.getDepth() >= 7 && random.nextInt(100) < 50) {
					room.setHasCrenellation(true);
					room.setHasMerlon(true);
					room.setHasParapet(true);
				} else {
