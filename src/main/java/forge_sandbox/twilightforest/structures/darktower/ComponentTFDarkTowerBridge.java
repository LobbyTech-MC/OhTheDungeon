...
public class ComponentTFDarkTowerBridge extends ComponentTFTowerWing {
	...
	protected ComponentTFDarkTowerBridge(TFFeature feature, int i, int x, int y, int z, int pSize, int pHeight,
			BlockFace direction) {
	...
	public boolean makeTowerWing(List<StructureTFComponent> list, Random rand, int index, int x, int y, int z,
		...
		ComponentTFTowerWing wing = new ComponentTFDarkTowerWing(getFeatureType(), index, dx[0], dx[1], dx[2], wingSize,
	...
	public StructureBoundingBox getWingBB() {
		...
		return StructureTFComponentOld.getComponentToAddBoundingBox(dest[0], dest[1], dest[2], 0, 0, 0, dSize - 1,
				dHeight - 1, dSize - 1, this.getCoordBaseMode());
	}
