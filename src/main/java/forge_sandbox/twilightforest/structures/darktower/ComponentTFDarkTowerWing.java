...
public class ComponentTFDarkTowerWing extends ComponentTFTowerWing {
	...
	protected ComponentTFDarkTowerWing(TFFeature feature, int i, int x, int y, int z, int pSize, int pHeight,
			BlockFace direction) {
	...
	public void buildComponent(StructureTFComponent parent, List<StructureTFComponent> list, Random rand) {
		...
		if (size > 10) {
			...
			for (Rotation direction : RotationUtil.ROTATIONS) {
				...
				boolean madeWing = makeTowerWing(list, rand, this.getComponentType(), dest[0], dest[1], dest[2],
						size - 2, childHeight, direction);
...
//        }
	...
	public void makeABeard(StructureTFComponent parent, List<StructureTFComponent> list, Random rand) {
		ComponentTFDarkTowerBeard beard = new ComponentTFDarkTowerBeard(getFeatureType(), this.getComponentType() + 1,
				this);
		list.add(beard);
	...
	public boolean makeTowerWing(List<StructureTFComponent> list, Random rand, int index, int x, int y, int z,
		...
		ComponentTFDarkTowerBridge bridge = new ComponentTFDarkTowerBridge(getFeatureType(), index, dx[0], dx[1], dx[2],
	...
	protected boolean makeTowerBalcony(List<StructureTFComponent> list, Random rand, int index, int x, int y, int z,
			Rotation rotation) {
		BlockFace direction = getStructureRelativeRotation(rotation);
		int[] dx = offsetTowerCoords(x, y, z, 5, direction);

		ComponentTFDarkTowerBalcony balcony = new ComponentTFDarkTowerBalcony(getFeatureType(), index, dx[0], dx[1],
				dx[2], direction);
		// check to see if it intersects something already there
	 ...
	 */
	protected void destroyTower(AsyncWorldEditor world, Random decoRNG, int x, int y, int z, int amount,
	...
	private void netherTransformBlob(AsyncWorldEditor world, Random inRand, int sx, int sy, int sz, int rad,
			StructureBoundingBox sbb) {
	...
	private void testAndChangeToNetherrack(AsyncWorldEditor world, Random rand, int x, int y, int z,
			StructureBoundingBox sbb) {
	 ...
	 */
	private void drawBlob(AsyncWorldEditor world, int sx, int sy, int sz, int rad, BlockData state,
			StructureBoundingBox sbb) {
		// then trace out a quadrant
		for (byte dx = 0; dx <= rad; dx++) {
			for (byte dy = 0; dy <= rad; dy++) {
	 ...
	 */
	protected void makeHalfFloor(AsyncWorldEditor world, StructureBoundingBox sbb, Rotation rotation, int y,
			int spacing) {
	 ...
	 */
	protected void makeFullFloor(AsyncWorldEditor world, StructureBoundingBox sbb, Rotation rotation, int y,
			int spacing) {
		// half floor
	 ...
	 * @param myDeco
	 */
	protected void decorateTreasureRoom(AsyncWorldEditor world, StructureBoundingBox sbb, Rotation rotation, int y,
		...
		placeTreasureAtCurrentPosition(world, null, x, y + 2, z,
				this.isKeyTower() ? TFTreasure.darktower_key : TFTreasure.darktower_cache, sbb);
	}

	private void decorateSpawner(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, Rotation rotation,
			int y) {
	 ...
	 */
	private void decorateLounge(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, Rotation rotation,
			int y) {
		int cx = this.size > 9 ? 9 : 7;
		int cz = this.size > 9 ? 4 : 3;

		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, false), cx, y + 1, cz + 0,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.WEST, rotation, false), cx, y + 1, cz + 1,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.NORTH, rotation, false), cx, y + 1, cz + 2,
				rotation, sbb);

		cx = this.size > 9 ? 5 : 3;

		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, true), cx, y + 1, cz + 0,
				rotation, sbb);
		setBlockStateRotated(world, getSlabState(Bukkit.createBlockData(Material.SPRUCE_SLAB), Slab.Type.TOP), cx,
				y + 1, cz + 1, rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.NORTH, rotation, true), cx, y + 1, cz + 2,
				rotation, sbb);
	}

	/**
	 * Decorate with a pressure plate triggered reappearing floor. Only suitable for
	 * small towers
	 */
	private void decorateReappearingFloor(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb,
	 ...
	 */
	private void decorateExperimentLamp(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb,
			Rotation rotation, int y) {
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.NORTH, rotation, false), cx, y + 1,
				cz + 2, rotation, sbb);
		setBlockStateRotated(world, deco.accentState, cx, y + 3, cz - 1, rotation, sbb);
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.SOUTH, rotation, true), cx, y + 3,
				cz - 2, rotation, sbb);
	}

	@SuppressWarnings("deprecation")
	protected static BlockData getLeverState(BlockData initialState, BlockFace direction, Rotation rotation,
			boolean isPowered) {
		return getLeverState(initialState, direction, Switch.Face.WALL, rotation, isPowered);
	}

	@SuppressWarnings("deprecation")
	protected static BlockData getLeverState(BlockData initialState, BlockFace direction, Switch.Face face,
			Rotation rotation, boolean isPowered) {
	 ...
	 */
	private void decorateExperimentPulser(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb,
			Rotation rotation, int y) {
	 ...
	 */
	private void decorateLibrary(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, Rotation rotation,
			int y) {
	...
	protected void makeSmallBookshelf(AsyncWorldEditor world, StructureBoundingBox sbb, Rotation rotation, int y,
			int bx, int bz) {
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.NORTH, rotation, false), bx, y + 1, bz + 0,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.NORTH, rotation, true), bx, y + 2, bz + 0,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, false), bx, y + 1, bz + 3,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, true), bx, y + 2, bz + 3,
				rotation, sbb);
	 ...
	 */
	@SuppressWarnings("deprecation")
	private void decoratePuzzleChest(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, Rotation rotation,
			int y) {
		int x = this.size > 9 ? 4 : 3;
		int z = this.size > 9 ? 5 : 4;
		// pillar frameframe
		...
		setBlockStateRotated(world, getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.WEST,
				Switch.Face.WALL, rotation, false), x + 2, y + 3, z - 2, rotation, sbb);
	 ...
	 */
	protected void makePillarFrame(AsyncWorldEditor world, StructureBoundingBox sbb, StructureTFDecorator myDeco,
			Rotation rotation, int x, int y, int z, boolean fenced) {
		makePillarFrame(world, sbb, myDeco, rotation, x, y, z, 3, 3, 3, fenced);
	}

	/**
	 * Place one of the architectural features that I frequently overuse in my
	 * structures
	 */
	protected void makePillarFrame(AsyncWorldEditor world, StructureBoundingBox sbb, StructureTFDecorator myDeco,
			Rotation rotation, int x, int y, int z, int width, int height, int length, boolean fenced) {
		// fill in posts
	 ...
	 * Dark tower half floors
	 */
	protected void addStairsDown(AsyncWorldEditor world, StructureBoundingBox sbb, Rotation rotation, int y, int sz,
			int spacing) {
		// stairs
		for (int i = 0; i < spacing; i++) {
			int sx = size - 3 - i;

			this.setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.WEST, rotation, false), sx, y - i,
					sz, rotation, sbb);
	 ...
	 */
	protected void addSmallTimberBeams(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, int bottom,
			int top) {
	 ...
	 */
	protected void makeTimberFloor(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, Rotation rotation,
			int y, int spacing) {
	 ...
	 */
	protected void makeSmallTimberBeams(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb,
		...
		/*
		 * int beamMetaNS = ((this.coordBaseMode + rotation) % 2 == 0) ? 4 : 8; int
		 * beamMetaEW = (beamMetaNS == 4) ? 8 : 4; int beamMetaUD = 0;
		 */
...
//        setBlockState(world, Blocks.LEVER, 7, lx, y + 1, lz, sbb);
	...
	/**
	 * Utility function to pick a random number between two values, excluding three
	 * specified values
	 */
	...
	/**
	 * Utility function for beam maze that checks if we should build a beam all the
	 * way down -- is there a valid spot to end it?
	 */
	protected boolean checkPost(AsyncWorldEditor world, int x, int y, int z, Rotation rotation,
			StructureBoundingBox sbb) {
	...
	/**
	 * Generate walls for the tower with the distinct pattern of blocks and accent
	 * blocks
	 *
	 * @param rand
	 */
	protected void makeEncasedWalls(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, int minX, int minY,
...
//                        }
	...
	/**
	 * Gets a random position in the specified direction that connects to a floor
	...
	/**
	 * Add an opening where we keep track of the kind of opening TODO: we could make
	 * a type of object that stores all these values TODO: also use an Enum for the
	 * kinds of openings?
	 */
...
//        }
	 ...
	 */
	protected void makeReappearingDoorOpening(AsyncWorldEditor world, int dx, int dy, int dz,
			StructureBoundingBox sbb) {
//        // nullify sky light
//        nullifySkyLightAtCurrentPosition(world, dx - 3, dy - 1, dz - 3, dx + 3, dy + 3, dz + 3);
//
