...
public class ComponentTFDarkTowerMain extends ComponentTFDarkTowerWing {
	...
	public ComponentTFDarkTowerMain(TFFeature feature, AsyncWorldEditor world, Random rand, int index, int x, int y,
			int z) {
		this(feature, world, rand, index, x + 10, y, z + 10, BlockFace.NORTH);
	}

	public ComponentTFDarkTowerMain(TFFeature feature, AsyncWorldEditor world, Random rand, int index, int x, int y,
			int z, BlockFace rotation) {
	...
	public void buildComponent(StructureTFComponent parent, List<StructureTFComponent> list, Random rand) {
		...
		if (this.getComponentType() < 2) {
			...
			for (Rotation rotation : RotationUtil.ROTATIONS) {
				...
				makeTowerWing(list, rand, this.getComponentType(), dest[0], dest[1], dest[2], 11, childHeight,
						rotation);
			}
		} else {
...
//                    TwilightForestMod.LOGGER.warn("Dark forest tower could not find four small towers to place keys in.");
	 ...
	 */
	private boolean makeEntranceTower(List<StructureTFComponent> list, Random rand, int index, int x, int y, int z,
			int childSize, int childHeight, Rotation rotation) {
		BlockFace direction = getStructureRelativeRotation(rotation);
		int[] dx = offsetTowerCoords(x, y, z, 5, direction);

		ComponentTFDarkTowerBridge bridge = new ComponentTFDarkTowerEntranceBridge(getFeatureType(), index, dx[0],
				dx[1], dx[2], childSize, childHeight, direction);
	 ...
	 */
	private boolean makeNewLargeTower(List<StructureTFComponent> list, Random rand, int index, int x, int y, int z,
			Rotation rotation) {
		...
		ComponentTFDarkTowerMainBridge bridge = new ComponentTFDarkTowerMainBridge(getFeatureType(), index, dx[0],
				dx[1], dx[2], wingSize, wingHeight, direction);
		// if I'm doing this right, the main towers can't intersect
		list.add(bridge);
		bridge.buildComponent(this, list, rand);
		// lock the door!
	 ...
	 */
	private boolean makeBossTrapWing(List<StructureTFComponent> list, Random rand, int index, int x, int y, int z,
			Rotation rotation) {
		...
		ComponentTFDarkTowerBossBridge bridge = new ComponentTFDarkTowerBossBridge(getFeatureType(), index, dx[0],
				dx[1], dx[2], wingSize, wingHeight, direction);
		// if I'm doing this right, the main towers can't intersect
		list.add(bridge);
		bridge.buildComponent(this, list, rand);
	 ...
	 */
	protected void addThreeQuarterFloors(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, int bottom,
			int top) {
	 ...
	 * Add a bunch of 3/4 floors
	 */
	protected void addThreeQuarterFloorsDecorateBoss(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb,
			int bottom, int top) {
	...
	private void decorateFloor(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, Rotation rotation,
			int y, boolean isBottom, boolean isTop) {
	 ...
	 */
	protected void makeThreeQuarterFloor(AsyncWorldEditor world, StructureBoundingBox sbb, Rotation rotation, int y,
...
//    }
	...
	protected void makeLargeStairsUp(AsyncWorldEditor world, StructureBoundingBox sbb, Rotation rotation, int y) {
		...
		for (int i = 0; i < 5; i++) {
			...
			setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, false), 1, sy, z,
					rotation, sbb);
			setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, false), 2, sy, z,
					rotation, sbb);
			...
			if (i > 0 && i < 4) {
				...
				setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.EAST, rotation, false), 3, sy, z,
						rotation, sbb);
			}
		}
	}

	private void decorateReappearingMaze(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb,
			Rotation rotation, int y) {
		// make maze object
		int mazeSize = 6;
...
//        maze.doorBlockState = TFBlocks.tower_device.getDefaultState().withProperty(BlockTFTowerDevice.VARIANT, TowerDeviceVariant.REAPPEARING_INACTIVE);
	 ...
	 */
	protected void decorateMazeDeadEnds(AsyncWorldEditor world, Random decoRNG, TFMaze maze, int y, Rotation rotation,
	 ...
	 */
	private void decorateDeadEnd(AsyncWorldEditor world, Random decoRNG, TFMaze maze, int mx, int y, int mz, int facing,
			Rotation rotation, StructureBoundingBox sbb) {
		int x = mx * 3 + 1;
		int z = mz * 3 + 1;
	...
	private void decorateUnbuilderMaze(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb,
			Rotation rotation, int y) {
...
//        setBlockStateRotated(world, antiBuilderBlockState, 5, y + 3, 13, rotation, sbb);
	...
	private void decorateLounge(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, Rotation rotation,
			int y) {
		...
		this.fillBlocksRotated(world, sbb, 12, y + 1, 2, 12, y + 1, 6,
				getStairState(deco.stairState, BlockFace.WEST, rotation, false), rotation);
		this.fillBlocksRotated(world, sbb, 12, y + 1, 6, 16, y + 1, 6,
				getStairState(deco.stairState, BlockFace.SOUTH, rotation, false), rotation);
		...
		this.fillBlocksRotated(world, sbb, 11, y + 1, 17, 12, y + 4, 17, Bukkit.createBlockData(Material.BOOKSHELF),
				rotation);
		this.fillBlocksRotated(world, sbb, 14, y + 1, 17, 15, y + 4, 17, Bukkit.createBlockData(Material.BOOKSHELF),
				rotation);
		this.fillBlocksRotated(world, sbb, 17, y + 1, 11, 17, y + 4, 12, Bukkit.createBlockData(Material.BOOKSHELF),
				rotation);
		this.fillBlocksRotated(world, sbb, 17, y + 1, 14, 17, y + 4, 15, Bukkit.createBlockData(Material.BOOKSHELF),
				rotation);

		// table
		this.setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, true), 13, y + 1, 14,
				rotation, sbb);
		this.setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.EAST, rotation, true), 14, y + 1, 14,
				rotation, sbb);
		this.setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.NORTH, rotation, true), 14, y + 1, 13,
				rotation, sbb);
		this.setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.WEST, rotation, true), 13, y + 1, 13,
				rotation, sbb);

		// chair 1
		this.setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.EAST, rotation, false), 11, y + 1, 13,
				rotation, sbb);

		// chair 2
		this.setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, false), 13, y + 1,
				11, rotation, sbb);

		// center lamp
		setBlockStateRotated(world, Bukkit.createBlockData(Material.REDSTONE_LAMP), 8, y + 3, 8, rotation, sbb);
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER),
						decoRNG.nextBoolean() ? BlockFace.EAST : BlockFace.SOUTH, Switch.Face.FLOOR, rotation, false),
				8, y + 2, 8, rotation, sbb);
	...
	private void makeDispenserPillar(AsyncWorldEditor world, StructureTFDecorator forgeDeco, int x, int y, int z,
			BlockFace stairMeta, Rotation rotation, StructureBoundingBox sbb) {
		this.setBlockStateRotated(world, getStairState(forgeDeco.stairState, stairMeta, rotation, true), x, y + 2, z,
				rotation, sbb);
		{
		...
		this.setBlockStateRotated(world, getStairState(forgeDeco.stairState, stairMeta, rotation, false), x, y + 4, z,
				rotation, sbb);
	}

	private void decorateBossSpawner(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, Rotation rotation,
			int y) {
//        this.setBlockStateRotated(world, TFBlocks.boss_spawner.getDefaultState().withProperty(BlockTFBossSpawner.VARIANT, BossVariant.UR_GHAST), 9, y + 4, 9, rotation, sbb);
	...
	@SuppressWarnings("deprecation")
	private void decorateExperiment(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, Rotation rotation,
			int y) {
...
//        placeItemFrameRotated(world, 17, y + 4, 5, rotation, WEST, new ItemStack(TFBlocks.tower_wood, 1, 1), sbb);
		...
		setBlockStateRotated(world, getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.EAST,
				Switch.Face.WALL, rotation, false), 13, y + 1, 17, rotation, sbb);
		{
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.SOUTH, rotation, false), 17, y + 1, 13,
				rotation, sbb);
		{
			Piston piston = (Piston) Bukkit.createBlockData(Material.PISTON);
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.EAST, rotation, true), 13, y + 1, 11,
				rotation, sbb);
		{
			Piston piston = (Piston) Bukkit.createBlockData(Material.PISTON);
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.EAST, rotation, false), 13, y + 1, 9,
				rotation, sbb);
		{
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.SOUTH, rotation, true), 11, y + 1, 13,
				rotation, sbb);
		{
			Piston piston = (Piston) Bukkit.createBlockData(Material.PISTON);
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER), BlockFace.SOUTH, rotation, false), 9, y + 1, 13,
				rotation, sbb);
		{
			Piston piston = (Piston) Bukkit.createBlockData(Material.STICKY_PISTON);
			piston.setFacing(BlockFace.WEST);
	...
	private void makeWoodPillar(AsyncWorldEditor world, StructureTFDecorator forgeDeco, int x, int y, int z,
			Rotation rotation, StructureBoundingBox sbb) {
...
//    }
	...
	private void decorateAquarium(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, Rotation rotation,
			int y) {
	...
	private void decorateForge(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, Rotation rotation,
			int y) {
...
//        forgeDeco.stairID = Blocks.STAIRCOMPACTCOBBLESTONE;
		...
		this.fillBlocksRotated(world, sbb, 12, y + 1, 12, 12, y + 1, 16,
				getStairState(deco.stairState, BlockFace.WEST, rotation, false), rotation);
		this.fillBlocksRotated(world, sbb, 12, y + 1, 12, 16, y + 1, 12,
				getStairState(deco.stairState, BlockFace.NORTH, rotation, false), rotation);
		...
		// framing pillars
		// this.makeStonePillar(world, forgeDeco, 12, y, 1, BlockFace.SOUTH, rotation,
		// sbb);
	...
	private void makeFurnacePillar(AsyncWorldEditor world, StructureTFDecorator forgeDeco, Random rand, int x, int y,
			int z, BlockFace direction, Rotation rotation, StructureBoundingBox sbb) {

		this.setBlockStateRotated(world, getStairState(deco.stairState, direction, rotation, true), x, y + 2, z,
				rotation, sbb);
...
//        }
		...
		this.setBlockStateRotated(world, getStairState(deco.stairState, direction, rotation, false), x, y + 4, z,
				rotation, sbb);
	}

	private void makeStonePillar(AsyncWorldEditor world, StructureTFDecorator forgeDeco, int x, int y, int z,
			BlockFace stairDirection, Rotation rotation, StructureBoundingBox sbb) {
	...
	private void makeFirePit(AsyncWorldEditor world, StructureTFDecorator myDeco, int x, int y, int z,
			Rotation rotation, StructureBoundingBox sbb) {
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.WEST, rotation, false), x - 1, y, z + 0,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.EAST, rotation, false), x + 1, y, z + 0,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.SOUTH, rotation, false), x + 0, y, z + 1,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.NORTH, rotation, false), x + 0, y, z - 1,
				rotation, sbb);
	...
	private void decorateNetherwart(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, Rotation rotation,
			int y, boolean isTop) {
...
//        this.fillBlocksRotated(world, sbb, 12, y + 3, 3, 15, y + 4, 6, 0, 0, rotation);
		...
		this.fillBlocksRotated(world, sbb, 13, y + 1, 10, 14, y + 1, 14, Bukkit.createBlockData(Material.SOUL_SAND),
				rotation);
		this.fillBlocksRotated(world, sbb, 13, y + 2, 10, 14, y + 2, 14, Bukkit.createBlockData(Material.NETHER_WART),
				rotation);
		this.fillBlocksRotated(world, sbb, 13, y + 4, 10, 14, y + 4, 14, Bukkit.createBlockData(Material.SOUL_SAND),
				rotation);
	...
	private void decorateBotanical(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, Rotation rotation,
			int y) {
		// main part
		...
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.EAST, rotation, true), 13, y + 1, 4,
				rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.WEST, rotation, true), 14, y + 1, 4,
				rotation, sbb);
		...
		// bench 2
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.EAST, rotation, true), 12, y + 1, 7,
				rotation, sbb);
		setBlockStateRotated(world, slab, 13, y + 1, 7, rotation, sbb);
		setBlockStateRotated(world, slab, 14, y + 1, 7, rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.WEST, rotation, true), 15, y + 1, 7,
				rotation, sbb);

		// bench 2
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.EAST, rotation, true), 12, y + 1, 10,
				rotation, sbb);
		setBlockStateRotated(world, slab, 13, y + 1, 10, rotation, sbb);
		setBlockStateRotated(world, slab, 14, y + 1, 10, rotation, sbb);
		setBlockStateRotated(world, getStairState(deco.stairState, BlockFace.WEST, rotation, true), 15, y + 1, 10,
				rotation, sbb);
	...
	private void placeTreePlanter(AsyncWorldEditor world, int treeNum, int x, int y, int z, Rotation rotation,
			StructureBoundingBox sbb) {
...
//        }
	...
	private void placeRandomPlant(AsyncWorldEditor world, Random decoRNG, int x, int y, int z, Rotation rotation,
			StructureBoundingBox sbb) {
...
//        setBlockStateRotated(world, flowerPotState, x, y, z, rotation, sbb);
	...
	private void makeBottomEntrance(AsyncWorldEditor world, Random decoRNG, StructureBoundingBox sbb, Rotation rotation,
			int y) {
	...
	protected void addTimberMaze(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, int bottom, int top) {
		...
		for (int y = bottom; y < top; y += spacing) {
			...
			makeTimberBeams(world, rand, sbb, floorside, y, y == bottom && bottom != spacing, y >= (top - spacing),
					top);
		}
	}
	 ...
	 */
	@SuppressWarnings("deprecation")
	protected void makeTimberBeams(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, Rotation rotation,
			int y, boolean isBottom, boolean isTop, int top) {
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER),
						rand.nextBoolean() ? BlockFace.EAST : BlockFace.NORTH, Switch.Face.FLOOR, rotation, false),
				lx, y + 1, lz, rotation, sbb);
	}

	// TODO : make this work with async world editor
	/**
	 * Make a mini ghast spawner and then set the spawn range and max entities for
	 * that spawner
	 */
	private void makeMiniGhastSpawner(AsyncWorldEditor world, Random rand, int y, int sx, int sz,
			StructureBoundingBox sbb) {
	}
	 ...
	 */
	protected void addBuilderPlatforms(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb, int bottom,
			int top) {
		...
		for (int y = bottom; y < (top - spacing); y += spacing) {
			makeBuilderPlatforms(world, rand, sbb, floorside, y, y == bottom && bottom != spacing,
					y >= (top - spacing));
			floorside = floorside.add(Rotation.CLOCKWISE_90);
	 ...
	 */
	protected void makeBuilderPlatforms(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb,
			Rotation rotation, int y, boolean bottom, boolean top) {
	 ...
	 */
	@SuppressWarnings("deprecation")
	private void addTopBuilderPlatform(AsyncWorldEditor world, Random rand, int bottom, int top, int spacing,
			StructureBoundingBox sbb) {
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER),
						rand.nextBoolean() ? BlockFace.SOUTH : BlockFace.EAST, Switch.Face.CEILING, rotation, false),
				7, top - spacing + 1, 11, rotation, sbb);

	}

	@SuppressWarnings("deprecation")
	private void makeBuilderPlatform(AsyncWorldEditor world, Random rand, Rotation rotation, int y, int z, boolean hole,
			StructureBoundingBox sbb) {
		// top platform
		...
		setBlockStateRotated(world,
				getLeverState(Bukkit.createBlockData(Material.LEVER),
						rand.nextBoolean() ? BlockFace.SOUTH : BlockFace.EAST, Switch.Face.CEILING, rotation, false),
				2, y + 1, z + 0, rotation, sbb);
	}

	/**
	 * Make a cluster of redstone lamps w/ switches as decoration for the empty
	 * areas in towers
	 */
	@SuppressWarnings("deprecation")
	private void makeLampCluster(AsyncWorldEditor world, Random rand, int sx, int y, int sz, Rotation rotation,
			StructureBoundingBox sbb) {
		int radius = 4;
		...
		for (int i = 0; i < 5; i++) {
			...
			for (int move = 0; move < 10; move++) {
				...
				if (getBlockStateFromPosRotated(world, lx, ly, lz, sbb, rotation)
						.getMaterial() != Material.REDSTONE_LAMP) {
					...
					setBlockStateRotated(world, getLeverState(Bukkit.createBlockData(Material.LEVER), switch_face,
							orientation, rotation, false), lx, ly, lz, rotation, sbb);
					break;
				}
			}
		}
