 ...
 * <p>
 * The cells are at odd numbered x and y values, and the walls are at even
 * numbered ones. This does make the storage slightly inefficient, but oh wells.
 *
 * @author Ben
 */
public class TFMaze {
	...
	public StructureTFComponent.BlockSelector wallBlocks;

	public BlockData wallBlockState;

	public BlockData headBlockState;

	public BlockData rootBlockState;

	public BlockData pillarBlockState;

	public BlockData doorBlockState;
	public float doorRarity;

	public BlockData torchBlockState;
	...
	public TFMaze(int cellsWidth, int cellsDepth) {
		...
		head = 0;
		roots = 0;
		wallBlockState = Bukkit.createBlockData(Material.CHISELED_STONE_BRICKS);
		rootBlockState = Bukkit.createBlockData(Material.STONE_BRICKS);
		torchBlockState = Bukkit.createBlockData(Material.TORCH);
	...
	/**
	 * Gets the wall value, or OUT_OF_BOUNDS if the area is out of bounds or the
	 * coordinates are not orthogonally adjacent.
	 */
...
//        TwilightForestMod.LOGGER.info("Wall check out of bounds; s = {}, {}; d = {}, {}", sx, sz, dx, dz);
	 ...
	 */
	public void copyToWorld(AsyncWorldEditor world, int dx, int dy, int dz) {
		worldX = dx;
		worldY = dy;
		worldZ = dz;
...
//                            (new TFGenCanopyTree()).generate(world, rand, new BlockPos(mdx, dy, mdz));
	 ...
	 */
	public void carveToWorld(AsyncWorldEditor world, int dx, int dy, int dz) {
		worldX = dx;
		worldY = dy;
		worldZ = dz;
	 ...
	 */
	public void copyToStructure(AsyncWorldEditor world, int dx, int dy, int dz, StructureTFComponentOld component,
...
//                            (new TFGenCanopyTree()).generate(world, rand, mdx, dy, mdz);
		...
		for (int x = 0; x < rawWidth; x++) {
			for (int z = 0; z < rawDepth; z++) {
				if (getRaw(x, z) == 0) {
					...
					if (isEven(x) && isEven(z)) {
						if (shouldTorch(x, z) && component.getBlockStateFromPos(world, mdx, mdy, mdz,
								sbb) == wallBlockState.getMaterial()) {
	...
	private void makeWallThing(AsyncWorldEditor world, int dy, StructureTFComponentOld component,
			StructureBoundingBox sbb, int mdx, int mdz, int even, int odd) {
		for (int y = 0; y < head; y++) {
			putHeadBlock(world, mdx + even, dy + tall + y, mdz + odd, component, sbb);
		}
		for (int y = 0; y < tall; y++) {
	 ...
	 */
	private void putPillarBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component,
			StructureBoundingBox sbb) {
	 ...
	 */
	private void putWallBlock(AsyncWorldEditor world, int x, int y, int z) {
	 ...
	 */
	private void putWallBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component,
			StructureBoundingBox sbb) {
		if (wallBlocks != null) {
	 ...
	 * Puts a wall block in the structure, at the specified structure coordinates.
	 */
	private void putDoorBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component,
			StructureBoundingBox sbb) {
		component.setBlockState(world, doorBlockState, x, y, z, sbb);
	}

	/**
	 * Carves a block into the world. TODO: check what's there? maybe only certain
	 * blocks?
	 */
	private void carveBlock(AsyncWorldEditor world, int x, int y, int z) {
		world.setBlockState(new BlockPos(x, y, z), Material.AIR, 2);
	}

	private void putHeadBlock(AsyncWorldEditor world, int x, int y, int z) {
		world.setBlockState(new BlockPos(x, y, z), headBlockState, 2);
	}

	private void putHeadBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component,
			StructureBoundingBox sbb) {
	 ...
	 */
	private void putRootBlock(AsyncWorldEditor world, int x, int y, int z) {
	 ...
	 */
	private void putRootBlock(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component,
			StructureBoundingBox sbb) {
	 ...
	 */
	private void putCanopyTree(AsyncWorldEditor world, int x, int y, int z, StructureTFComponentOld component,
			StructureBoundingBox sbb) {
		BlockPos pos = component.getBlockPosWithOffset(x, y, z);

		// only place it if we're actually generating the chunk the tree is in (or at
		// least the middle of the tree)
...
//            (new TFGenCanopyTree()).generate(world, rand, pos);
	 ...
	 */
	private void placeTorches(AsyncWorldEditor world) {
		...
		for (int x = 0; x < rawWidth; x++) {
			for (int z = 0; z < rawDepth; z++) {
				if (getRaw(x, z) == 0) {
					...
					if (isEven(x) && isEven(z)) {
						if (shouldTorch(x, z) && world.getBlockState(pos) == wallBlockState.getMaterial()) {
	...
	/**
	 * Should we put a torch here? Intended to be called on the in-between spots
	 * where x and y are even.
	 */
	public boolean shouldTorch(int rx, int rz) {
		// if there is out of bounds in any direction, no
		if (getRaw(rx + 1, rz) == OOB || getRaw(rx - 1, rz) == OOB || getRaw(rx, rz + 1) == OOB
				|| getRaw(rx, rz - 1) == OOB) {
			return false;
		}
	...
	/**
	 * Should we put a tree instead of a post? Essentially the answer is yes for the
	 * corners and the exits.
	 */
	...
	/**
	 * If the worldX is set properly, this returns where in that world the maze
	 * coordinate x lies
	 *
	 * @param x
	 * @return
	...
	/**
	 * If the worldZ is set properly, this returns where in that world the maze
	 * coordinate z lies
	 *
	 * @param z
	 * @return
	...
	/**
	 * Mark the cell as visited. If we have any unvisited neighbors, pick one
	 * randomly, carve the wall between them, then call this function on that
	 * neighbor.
	 *
	 * @param sx
	 * @param sz
	...
	public void rbGen(int sx, int sz) {
		...
		// the destination has run out of free spaces, let's try this square again, up
		// to 2 more times
		rbGen(sx, sz);
		rbGen(sx, sz);
	}
