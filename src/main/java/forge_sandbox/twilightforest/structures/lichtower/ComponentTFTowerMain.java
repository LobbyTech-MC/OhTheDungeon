...
public class ComponentTFTowerMain extends ComponentTFTowerWing {
	...
	public ComponentTFTowerMain(TFFeature feature, AsyncWorldEditor world, Random rand, int index, int x, int y,
			int z) {
	...
	public void buildComponent(StructureTFComponent parent, List<StructureTFComponent> list, Random rand) {
		...
		for (int i = 0; i < 4; i++) {
			for (final Rotation towerRotation : RotationUtil.ROTATIONS) {
				...
				if (rand.nextInt(3) == 0
						|| !makeTowerWing(list, rand, 1, dest[0], dest[1], dest[2], 5, childHeight, towerRotation)) {
	...
	/**
	 * Gets a random position in the specified direction that we can make an
	 * outbuilding at
	 */
	...
	public boolean makeTowerOutbuilding(List<StructureTFComponent> list, Random rand, int index, int x, int y, int z,
			int wingSize, int wingHeight, Rotation rotation) {
		BlockFace direction = getStructureRelativeRotation(rotation);
		int[] dx = offsetTowerCoords(x, y, z, wingSize, direction);
		ComponentTFTowerOutbuilding outbuilding = new ComponentTFTowerOutbuilding(getFeatureType(), index, dx[0], dx[1],
				dx[2], wingSize, wingHeight, direction);
...
//        spawnBossLater(world, size / 2, floorLevel + 2, size / 2, TFBoss.Lich);
	...
	protected void decorateLichChandelier(AsyncWorldEditor world, int floorLevel, StructureBoundingBox sbb) {
		...
		SpawnerBoss_Later.generate_later(world,
				new Coord(this.getXWithOffset(x, z), this.getYWithOffset(y), this.getZWithOffset(x, z)), TFBoss.Lich);

	}

	/**
	 * Cover the walls in the lich's room with paintings. How is this going to work,
	 * chunk by chunk?
	 */
	...
	/**
	 * Place up to 5 torches (with fence holders) on the wall, checking that they
	 * don't overlap any paintings or other torches
	 */
	protected void generateTorchesOnWall(AsyncWorldEditor world, Random rand, int floorLevel, BlockFace direction,
