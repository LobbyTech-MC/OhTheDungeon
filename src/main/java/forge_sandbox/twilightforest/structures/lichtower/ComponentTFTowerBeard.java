...
public class ComponentTFTowerBeard extends StructureTFComponentOld {
	...
	public ComponentTFTowerBeard(TFFeature feature, int i, ComponentTFTowerWing wing) {
		...
		this.boundingBox = new StructureBoundingBox(wing.getBoundingBox().minX + 1,
				wing.getBoundingBox().minY - this.height - 1, wing.getBoundingBox().minZ + 1,
				wing.getBoundingBox().maxX - 1, wing.getBoundingBox().minY - 1, wing.getBoundingBox().maxZ - 1);
	...
	private boolean makePyramidBeard(AsyncWorldEditor world, Random rand, StructureBoundingBox sbb) {
		for (int y = 0; y <= height; y++) {
			...
			fillWithRandomizedBlocks(world, sbb, min, height - y, min, max, height - y, max, false, rand,
